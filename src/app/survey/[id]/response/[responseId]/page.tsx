...
export default function ResponseDetailPage() {
  ...
  const currentIndex = mockResponses.findIndex((r) => r.id === responseId) + 1

  const handlePrevious = () => {
    const prevIndex = currentIndex - 2
    if (prevIndex >= 0) {
      router.push(`/survey/${surveyId}/response/${mockResponses[prevIndex].id}`)
    }
  }

  const handleNext = () => {
    if (currentIndex < mockResponses.length) {
      router.push(`/survey/${surveyId}/response/${mockResponses[currentIndex].id}`)
    }
  }
          ...
          <div className="bg-white border-b shadow-sm px-6 py-4">
            <div className="max-w-6xl mx-auto flex items-center justify-between">
              ...
              <div className="flex items-center gap-4">
                <Select 
                  value={responseId}
                  onValueChange={(value) => router.push(`/survey/${surveyId}/response/${value}`)}
                >
